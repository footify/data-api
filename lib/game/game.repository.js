...
function getAllGamesByUserId(userId) {
    return Friends.getFriendList(userId).then((friendList) => {
        ...
        const teamsPromises = _.map(friendList.accepted, friend => () => { return Friends.getByPlayer(friend._id); });
        console.log(teamsPromises);
        return Promise.all(teamsPromises).then((teams) => {
          console.log(teams);
          return _.chain(teams).flatten().map('_id').uniq().value()
        })
            .then((teamIds) => {
                console.log(teamIds);
                return Games.find({
