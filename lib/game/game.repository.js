...
function getAllGamesByUserId(userId) {
    return Friends.getFriendList(userId).then((friendList) => {
        const teamsPromises = _.map(friendList.accepted, friend => Teams.getByPlayer(friend._id));
        teamsPromises.push(Teams.getByPlayer(userId)); // On rajoute la team de l'user courant dans la liste
