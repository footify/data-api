...
function getAllGamesByPubId(pubId) {
  return Games.find({}).populate('teams team.player1 team.player2 babyfoot babyfoot.pub').sort({ start_date: -1 })
    .then((games) => {
...
function getAllGamesByBabyfootId(babyfootId) {
  return Games.find({ babyfoot: babyfootId }).populate('teams team.player1 team.player2').sort({ start_date: -1 });
}
